                            ...
                            if trendingresults.results.isEmpty{
                                ...
                                HStack{
                                    ForEach(homeViewModel.fetchedTrending!.results, id: \.id){
                                        result in NavigationLink(destination: MovieDetail(movieId: result.id, movieTitle: result.originalTitle!)){ OverlayContentView(releaseYear: result.firstAirDate ?? result.releaseDate ?? "", backdropPath: result.backdropPath ?? result.posterPath,movieTitle: result.originalTitle ?? result.originalName ?? result.name ?? "", mediaType: result.mediaType.rawValue )
                                        }
                                    }
                                }
                            }
                    ...
                    if let popularResults = homeViewModel.fetchedPopular{
                        if popularResults.results.isEmpty{
                            ...
                            LazyVGrid(columns: [GridItem(.adaptive(minimum: 170))]) {
                                ForEach(homeViewModel.fetchedPopular!.results, id: \.id) { result in NavigationLink(destination: MovieDetail(movieId: result.id, movieTitle: result.originalTitle)){
